...
for(y in years) {
  daily <- aqs_dailysummary_by_cbsa("42602", bdate = as.Date(paste0(y, "-01-01")), edate =  as.Date(paste0(y, "-12-31")), cbsa_code = 19740)
...
try1 %>% 
  arrange(date_local) %>% 
  write_csv(file = "~/OneDrive - The University of Colorado Denver/Websites/blog/peterson_blog/posts/did-denver-zero-fare-policy-work/no2_data-91-22-v3.csv")
